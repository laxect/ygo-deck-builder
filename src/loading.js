...
async function fetchCardDb() {
  ...
  try {
    // load card db
    let response = await noCacheFetch(
      "https://raye.mistivia.com/card_db_parts/version.json",
    );
    if (!response.ok) {
    ...
    if (localVer === data && (await getidxdbitem("card_db")) !== null) {
      ...
      localVer = data;
      response = await noCacheFetch(
        "https://raye.mistivia.com/card_db_parts/index.json",
      );
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
      data = await response.json();
      let tasks = data.map((i) =>
        noCacheFetch("https://raye.mistivia.com/card_db_parts/" + i),
      );
